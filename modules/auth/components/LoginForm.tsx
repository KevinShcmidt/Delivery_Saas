 ...
 * Utilise useActionState + signInAction (Server Action) pour que les cookies
 * soient set côté serveur avant la redirection — bonne pratique Next.js.
 */
...
export function LoginForm() {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      // Construire le FormData pour la Server Action
      const formData = new FormData();
      formData.set("email", credentials.email.trim().toLowerCase());
      formData.set("password", credentials.password);

      // useTransition — garde l'UI réactive pendant la Server Action
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <button
        ...
        disabled={isLoading}
        className="
          relative w-full overflow-hidden rounded-lg bg-orange-500 py-3.5 text-sm
          font-bold uppercase tracking-widest text-white shadow-lg
          shadow-orange-500/20 transition-all duration-200
          hover:bg-orange-400 hover:shadow-orange-500/30
